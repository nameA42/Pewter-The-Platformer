...
export class SelectionBox {
  ...
  // --- NESTING + FACTS (add these fields) ---
  public id: string; // stable identity for graph ops
  public parentId: string | null = null; // null = root
  public childIds: Set<string> = new Set(); // direct children

  // Facts: own facts (what this box directly placed/owns) and aggregated facts (own + descendants)
  public ownFacts: Record<string, number> = {};
  public aggFacts: Record<string, number> = {};

  private onFinalize?: (box: SelectionBox) => void; // <-- add this line with the other private fields
  ...
  constructor(
    ...
    onFinalize?: (box: SelectionBox) => void, // <-- NEW (optional)
  ) {
    this.scene = scene;
    ...
    // simple stable-ish id (good enough for editor sessions)
    this.id = `sel_${Date.now().toString(36)}_${Math.floor(Math.random() * 1e6).toString(36)}`;
    ...
    this.onFinalize = onFinalize; // <-- capture hook
  ...
  /** Inclusive tile rect info (x0..x1, y0..y1) + width/height in tiles */
  public getTileRect() {
  ...
  /** True if this selection fully contains the other selection (inclusive edges) */
  public containsBox(other: SelectionBox): boolean {
    const a = this.getTileRect();
    const b = other.getTileRect();
    return b.x0 >= a.x0 && b.y0 >= a.y0 && b.x1 <= a.x1 && b.y1 <= a.y1;
  }

  /** True if this selection covers a specific tile (tx, ty) */
  public coversTile(tx: number, ty: number): boolean {
    const r = this.getTileRect();
  ...
  public finalize() {
    ...
    // NEW: notify editor so it can index & link nesting
    if (this.onFinalize) {
      try {
