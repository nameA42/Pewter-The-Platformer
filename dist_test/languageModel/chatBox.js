...
exports.getCurrentChatHistory = getCurrentChatHistory;
exports.getDisplayChatHistory = getDisplayChatHistory;
exports.setActiveSelectionBox = setActiveSelectionBox;
exports.addChatMessage = addChatMessage;
exports.setBotResponding = setBotResponding;
exports.isBotResponding = isBotResponding;
exports.sendUserPrompt = sendUserPrompt;
exports.sendUserPromptWithContext = sendUserPromptWithContext;
exports.sendSystemMessage = sendSystemMessage;
exports.clearChatHistory = clearChatHistory;
const modelConnector_ts_1 = require("./modelConnector.ts");
...
function getDisplayChatHistory() {
  return currentChatHistory
...
function setActiveSelectionBox(box) {
  if (!box) {
  ...
  if (
    ...
    currentChatHistory.unshift(new messages_2.SystemMessage(sysPrompt));
    console.log(
 ...
 */
function setBotResponding(value) {
  botResponding = value;
}
/**
 * Check if the bot is currently processing a prompt.
 */
function isBotResponding() {
 ...
 */
async function sendUserPrompt(message) {
  ...
  try {
    const reply = await (0, modelConnector_ts_1.getChatResponse)(historyRef);
    const replyText = Array.isArray(reply.text)
      ? reply.text.join("\n")
      : String(reply.text);
    const aiMessage = new messages_1.AIMessage(replyText);
    ...
    const fallback = new messages_1.AIMessage("Error: " + errorMessage);
 ...
 */
async function sendUserPromptWithContext(userMessageText, hiddenContext) {
  const historyRef = currentChatHistory;
  // Push the user's visible message into the active history
  const userMessage = new messages_1.HumanMessage(userMessageText);
  historyRef.push(userMessage);
  setBotResponding(true);
  try {
    // Build a temporary history for the model that includes the hidden context
    const tempHistory = historyRef.slice();
    if (hiddenContext && hiddenContext.trim().length > 0) {
      tempHistory.push(
        new messages_1.HumanMessage(`[WORLD CONTEXT]: ${hiddenContext}`),
      );
    }
    const reply = await (0, modelConnector_ts_1.getChatResponse)(tempHistory);
    const replyText = Array.isArray(reply.text)
      ? reply.text.join("\n")
      : String(reply.text);
    const aiMessage = new messages_1.AIMessage(replyText);
...
async function sendSystemMessage(message) {
  const historyRef = currentChatHistory;
  const systemMessage = new messages_1.HumanMessage(message);
