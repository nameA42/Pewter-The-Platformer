...
import * as chatBox from "../languageModel/chatBox";
...
(async function runRegenerateTests() {
  ...
  const scene: any = Object.create(EditorScene.prototype);
  // minimal map
  scene.map = { width: 10, height: 10 };
  const placements: Array<{ x: number; y: number; tileIndex: number }> = [];
  ...
  scene.placeTile = (layer: any, x: number, y: number, tileIndex: number) => {
  ...
  const mockBox: any = {
    getStart: () => new Phaser.Math.Vector2(1, 1),
    getEnd: () => new Phaser.Math.Vector2(2, 2),
  ...
  let spyCalled = false;
  let spyArgs: any = null;
  (chatBox as any).sendUserPromptWithContext = async (
    userPrompt: string,
